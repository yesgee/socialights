...
describe('Action: addUserToRoom', function() {
  ...
  it('should add a user to a room', function(done){
    ...
    user.save(function(err) {
      ...
      room.save(function(err) {
        should.not.exist(err);
        api.specHelper.runAction('addUserToRoom', {user: user.id, room: room.id}, function(response) {
  ...
  it('should return an error for a non-existing room', function(done) {
    ...
    user.save(function(err) {
      should.not.exist(err);
      api.specHelper.runAction('addUserToRoom', { user: user.id
  ...
  it('should return an error for a non-existing user', function(done) {
    ...
    room.save(function(err) {
      should.not.exist(err);
      api.specHelper.runAction('addUserToRoom', { user: new api.mongo.ObjectID()
